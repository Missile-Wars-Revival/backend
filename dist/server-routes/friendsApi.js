...
async function getMutualFriends(currentUser) {
    ...
    for (const friendUsername of currentUser.friends) {
        const friend = await server_1.prisma.users.findUnique({
...
function setupFriendsApi(app) {
    ...
    app.get("/api/searchfriendsadded", async (req, res) => {
        ...
        try {
            ...
            const addedFriends = await server_1.prisma.users.findMany({
                where: {
                    username: {
            ...
            const formattedFriends = nonMutualFriends.map((friend) => ({ username: friend.username, updatedAt: friend.updatedAt }));
    ...
    app.get("/api/nearby", async (req, res) => {
        ...
        try {
            ...
            const mainUser = await server_1.prisma.users.findUnique({
                where: {
                    username: decoded.username,
                },
                select: { friends: true }
            ...
            // Fetch nearby users
            const nearbyUsers = await server_1.prisma.gameplayUser.findMany({
                where: {
                    AND: [
            ...
            const filteredNearbyUsers = nearbyUsers.filter((user) => {
                ...
                if (isNaN(userLatitude) || isNaN(userLongitude))
                    return false;
                const distance = geolib.getDistance({ latitude, longitude }, { latitude: userLatitude, longitude: userLongitude });
    ...
    app.get("/api/friends", async (req, res) => {
        ...
        try {
            ...
            if (user.friends.length > 0) {
                ...
                const friendsProfiles = await server_1.prisma.users.findMany({
                    where: {
                        username: {
                            in: user.friends,
                        },
    ...
    app.post("/api/addFriend", async (req, res) => {
        ...
        try {
            ...
            const friendUser = await server_1.prisma.users.findFirst({
            ...
            if (isMutualFriend) {
                await (0, notificationhelper_1.sendNotification)(friend, "Friend Accepted", `${user.username} has added you back!`, user.username);
            }
            else {
                await (0, notificationhelper_1.sendNotification)(friend, "Friend Request", `${user.username} has added you as a friend!`, user.username);
            }
...
exports.setupFriendsApi = setupFriendsApi;

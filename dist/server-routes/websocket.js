...
const middleearth = __importStar(require("middle-earth"));
const server_1 = require("../server");
const friendsApi_1 = require("./friendsApi");
const axios_1 = __importDefault(require("axios"));
function logVerbose(...items) {
    // Logs an item only if the VERBOSE_MODE env variable is set
    if (process.env.VERBOSE_MODE === "ON") {
        console.log(...items);
    }
}
async function updateOrCreateSession(userId, ip) {
    const existingSession = await server_1.prisma.sessions.findFirst({
        where: { userId: userId }
    });
    if (existingSession) {
        // If a session exists, update it
        await server_1.prisma.sessions.update({
        ...
        await server_1.prisma.sessions.create({
            data: {
                userId: userId,
                lastLoginTime: new Date(),
                lastIp: ip || 'unknown'
    ...
    app.ws("/", (ws, req) => {
        authenticate(ws, req).then((authResult) => {
            ...
            const sendPeriodicData = async () => {
                await refreshCache();
                const currentUser = await server_1.prisma.users.findUnique({
                ...
                const mutualFriendsUsernames = await (0, friendsApi_1.getMutualFriends)(currentUser);
            ...
            const sendLessPeriodicData = async () => {
                try {
                    ...
                    const inventory = await server_1.prisma.inventoryItem.findMany({
                        where: {
                            userId: user.id,
                        },
                        select: {
                    ...
                    //friends data
                    const friendsData = await server_1.prisma.users.findMany({
                        where: {
                            username: {
                    ...
                    const allGameplayUsers = await server_1.prisma.gameplayUser.findMany({
                        where: whereClause,
                        include: { Locations: true }
                    });
                    const calculateTransportStatus = async (currentLocation, previousLocation, username) => {
                        if (!previousLocation)
                            return 'walking';
                        const timeDiff = (currentLocation.updatedAt.getTime() - previousLocation.updatedAt.getTime()) / 1000; // in seconds
                        if (timeDiff <= 0)
                            return 'walking'; // Avoid division by zero or negative time
                        const distance = calculateDistance({ latitude: parseFloat(previousLocation.latitude), longitude: parseFloat(previousLocation.longitude) }, { latitude: parseFloat(currentLocation.latitude), longitude: parseFloat(currentLocation.longitude) });
                        ...
                        if (speed > 70)
                            transportStatus = 'plane'; // Approx. 252 km/h
                        else if (speed > 23)
                            transportStatus = 'highspeed'; // Approx. 83 km/h
                        else if (speed > 8)
                            transportStatus = 'car'; // Approx. 28.8 km/h
                        else if (speed > 1.5)
                            transportStatus = 'bicycle'; // Approx. 5.4 km/h
                        // Check if in sea
                        ...
                        await server_1.prisma.locations.update({
                            where: { username: username },
                    ...
                    const locations = await Promise.all(allGameplayUsers.map(async (gpu) => {
                        ...
                        if (!currentLocation)
                            return null; // Skip this user if no location data
                        const previousLocation = currentLocation.previousLat && currentLocation.previousLong
                            ? {
...
async function isInSea(location) {
    const query = `
    [out:json];
  ...
  `;
    ...
    while (attempt < maxRetries) {
        try {
            const response = await axios_1.default.get(`https://overpass-api.de/api/interpreter?data=${encodeURIComponent(query)}`);
...
async function handlePlayerLocation(ws, msg, username) {
    ...
    const now = new Date();
    try {
        const lastLocation = await server_1.prisma.locations.findFirst({
            where: { username: username },
        ...
        if (lastLocation) {
            ...
            // Create new location
            await server_1.prisma.locations.create({
                data: {
                    username: username,
                    latitude: locationData.latitude.toString(),
                    longitude: locationData.longitude.toString(),

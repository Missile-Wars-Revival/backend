...
function setupLeagueApi(app) {
    app.get("/api/topleagues", async (req, res) => {
        ...
        try {
            ...
            const leagues = await server_1.prisma.league.findMany({
                include: {
                    _count: {
                        select: { players: true }
                    }
    ...
    app.get("/api/leagues/current", async (req, res) => {
        ...
        try {
            ...
            let user = await server_1.prisma.gameplayUser.findUnique({
            ...
            if (!user.league) {
                ...
                user = await server_1.prisma.gameplayUser.findUnique({
                    where: { id: user.id },
                    include: { league: true }
                });
                if (!user || !user.league) {
    ...
    app.get("/api/leagues/players", async (req, res) => {
        const { token } = req.query;
        if (!token) {
            return res.status(400).json({ success: false, message: "Missing token" });
        }
        try {
            ...
            const players = await server_1.prisma.gameplayUser.findMany({
    ...
    app.get("/api/top100players", async (req, res) => {
        const { token } = req.query;
        if (!token) {
            return res.status(400).json({ success: false, message: "Missing token" });
        }
        try {
            ...
            const top100Players = await server_1.prisma.gameplayUser.findMany({
                select: {
                    id: true,
                    username: true,
                    rankPoints: true,
...
async function assignUserToLeague(userId) {
    ...
    const availableLeagues = await server_1.prisma.league.findMany({
        where: {
            tier,
            division,
        },
    ...
    if (!league) {
        ...
        const lastLeague = await server_1.prisma.league.findFirst({
            where: { tier, division },
            orderBy: { number: 'desc' }
        });
        const newLeagueNumber = lastLeague ? lastLeague.number + 1 : 1;
        league = await server_1.prisma.league.create({
            data: {
                tier,
                division,
...
async function balanceLeaguesIfNeeded(tx, tier, division) {
    ...
    const overfilledLeagues = leagues.filter((l) => l._count.players > MAX_PLAYERS_PER_LEAGUE + SOFT_LIMIT_BUFFER);
    const underfilledLeagues = leagues.filter((l) => l._count.players < MIN_PLAYERS_PER_LEAGUE);
    ...
    for (const league of overfilledLeagues) {
        ...
        for (const player of playersToMove) {
            const targetLeague = leagues.find((l) => l._count.players < MAX_PLAYERS_PER_LEAGUE);
            if (targetLeague) {
                await tx.gameplayUser.update({
                    where: { id: player.id },
    ...
    for (const league of underfilledLeagues) {
        if (league._count.players < MIN_PLAYERS_PER_LEAGUE) {
            ...
            for (const player of playersToMove) {
                const targetLeague = leagues.find((l) => l.id !== league.id && l._count.players < MAX_PLAYERS_PER_LEAGUE);
                if (targetLeague) {
                    await tx.gameplayUser.update({
                        where: { id: player.id },
                        data: { leagueId: targetLeague.id }
...
async function checkAndUpdateUserLeagues() {
    console.log("Starting hourly league update check...");
    const users = await server_1.prisma.gameplayUser.findMany({
...
async function sendLeagueChangeNotification(username, oldLeague, newLeague, isPromotion) {
    ...
    await (0, notificationhelper_1.sendNotification)(username, title, body, 'Server');
}
async function getGlobalTopPlayer() {
    const topPlayer = await server_1.prisma.gameplayUser.findFirst({
        orderBy: { rankPoints: 'desc' },
        include: { league: true }
    });
    if (!topPlayer)
...
function getDivisionFromRankPoints(rankPoints) {

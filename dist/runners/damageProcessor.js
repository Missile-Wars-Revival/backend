...
exports.startDamageProcessing = exports.processDamage = void 0;
...
const processedMissiles = new Map();
const processedLandmines = new Map();
...
const processDamage = async () => {
    try {
        ...
        const [activeUsers, activeMissiles, activeLandmines, activeShields, allGameplayUsers] = await Promise.all([
            ...
            server_1.prisma.missile.findMany({ where: { status: 'Hit' } }),
        ...
        const gameplayUserMap = new Map(allGameplayUsers.map((u) => [u.username, { friendsOnly: u.friendsOnly }]));
        for (const user of activeUsers) {
            ...
            for (const missile of activeMissiles.filter((m) => usernamesToProcess.includes(m.sentBy))) {
                ...
                const distance = (0, entitymanagment_1.haversine)(userCoords.latitude.toString(), userCoords.longitude.toString(), missileCoords.latitude.toString(), missileCoords.longitude.toString());
            ...
            for (const landmine of activeLandmines.filter((l) => usernamesToProcess.includes(l.placedBy))) {
                ...
                const distance = (0, entitymanagment_1.haversine)(userCoords.latitude.toString(), userCoords.longitude.toString(), landmineCoords.latitude.toString(), landmineCoords.longitude.toString());
...
async function determineUsernamesToProcess(username, gameplayUserMap) {
    ...
    try {
        ...
        userFriends = (user === null || user === void 0 ? void 0 : user.friends) || [];
    }
    catch (error) {
    ...
    let mutualFriends;
    try {
        mutualFriends = await server_1.prisma.users.findMany({
            where: {
...
function isUserProtectedByShield(userCoords, shields) {
    for (const shield of shields) {
        const shieldCoords = { latitude: parseFloat(shield.locLat), longitude: parseFloat(shield.locLong) };
        const distance = (0, entitymanagment_1.haversine)(userCoords.latitude.toString(), userCoords.longitude.toString(), shieldCoords.latitude.toString(), shieldCoords.longitude.toString());
...
async function handleMissileDamage(user, missile) {
...
async function handleLandmineDamage(user, landmine) {
    ...
    if (!processedLandmines.get(user.username).has(landmine.id)) {
        ...
        await (0, notificationhelper_1.sendNotification)(user.username, "Landmine Damage!", warningMessage, "server");
...
async function applyDamage(user, damage, attackerUsername, damageSource, receivedType, entityId) {
    ...
    try {
        const applyDamageRecursively = async () => {
            ...
            const [currentUser, activeShields] = await Promise.all([
                server_1.prisma.gameplayUser.findUnique({
                    where: { id: user.id },
                    include: { Locations: true }
                }),
                server_1.prisma.other.findMany({
            ...
            if (lastDeath && Date.now() - lastDeath < FIVE_MINUTES) {
                ...
                await (0, notificationhelper_1.sendNotification)(attackerUsername, "Attack Ineffective", gracePeriodMessage, "server");
                return;
            }
            ...
            if (damageSource === 'missile') {
                ...
                const missile = await server_1.prisma.missile.findUnique({
            ...
            if (damageSource === 'landmine') {
                ...
                const landmine = await server_1.prisma.landmine.findUnique({
                    where: { id: entityId }
                });
                if (landmine) {
                    const landmineCoords = { latitude: parseFloat(landmine.locLat), longitude: parseFloat(landmine.locLong) };
            ...
            if (damageSource === 'landmine') {
                const landmineType = await server_1.prisma.landmineType.findUnique({
                ...
                const missileType = await server_1.prisma.missileType.findUnique({
                    where: { name: receivedType },
                });
                if (missileType) {
            ...
            await server_1.prisma.$transaction(async (prisma) => {
                ...
                if (updatedUser.health <= 0 && updatedUser.isAlive) {
                    ...
                    await (0, notificationhelper_1.sendNotification)(user.username, "Eliminated!", eliminationMessage, attackerUsername);
                    ...
                    await (0, notificationhelper_1.sendNotification)(user.username, "Grace Period Activated", gracePeriodMessage, "server");
                    ...
                    await (0, notificationhelper_1.sendNotification)(user.username, "Damaged!", damageMessage, attackerUsername);
        ...
        if (damageSource === 'landmine') {
            ...
            await (0, notificationhelper_1.sendNotification)(user.username, "Missile Damage!", initialMissileMessage, attackerUsername);
            setTimeout(applyDamageRecursively, 30000);
        }
    }
    catch (error) {
...
async function updateDeathStatistic(userId, prisma) {
...
async function updateKillStatistic(userId, prisma) {
...
const startDamageProcessing = () => {
    setInterval(() => {
        (0, exports.processDamage)();
        cleanupProcessedEntities();
    }, PROCESS_INTERVAL);
    // console.log('Damage processing started, running every 15 seconds');
};
exports.startDamageProcessing = startDamageProcessing;
